...
interface ProductFilterProps {
    ...
    setSelectedFunction?: (val: string | null) => void;
...
export default function ProductFilter({
    ...
    const isCategoryDisabled = !!currentCategorySlug;
    const isCollectionDisabled = !!currentCollectionSlug;
    const isFunctionDisabled = !!currentFunctionSlug;

    // Pick initial expanded section from non-disabled sections
    const getInitialExpanded = () => {
        if (!isCategoryDisabled && setSelectedCategory) return "category";
        if (!isCollectionDisabled && setSelectedCollection) return "collection";
        if (!isFunctionDisabled && setSelectedFunction) return "function";
        return "price";
    };
    ...
    const hasActiveFilters =
        ...
        (selectedFunction !== undefined && selectedFunction !== null) ||
        priceRange !== null;
        ...
        <div className="relative flex items-center gap-4" ref={filterRef}>
            ...
            {filterOpen && (
                <div className="absolute right-0 sm:left-0 top-full z-[999] mt-2 w-[290px] border border-[#ede0c4] bg-white shadow-[0_12px_40px_rgba(196,168,79,0.15)] rounded-[3px]">
                    <div className="max-h-[65vh] overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-[#c4a84f]/40 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-[#faf7f2]">
                        ...
                        <div className="border-b border-[#f0e8d6]">
                            ...
                            {!isCategoryDisabled && expandedSection === "category" && (
                                <div className="filter-accordion-content px-4 pb-3 flex flex-col gap-1.5">
                                    {setSelectedCategory ? (
                                        // In-place radio filters
                                        <div className="flex flex-col gap-1.5 max-h-[200px] overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-[#c4a84f]/40 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-[#faf7f2]">
                                        ...
                                        // Link navigation
                                        <div className="flex flex-col gap-0.5 max-h-[200px] overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-[#c4a84f]/40 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-[#faf7f2]">
                                            {categories.length === 0 ? (
                        ...
                        <div className="border-b border-[#f0e8d6]">
                            ...
                            {!isCollectionDisabled && expandedSection === "collection" && (
                                <div className="filter-accordion-content px-4 pb-3 flex flex-col gap-1.5">
                                    {setSelectedCollection ? (
                                        // In-place radio filters
                                        <div className="flex flex-col gap-1.5 max-h-[200px] overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-[#c4a84f]/40 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-[#faf7f2]">
                        ...
                        <div className="border-b border-[#f0e8d6]">
                            ...
                            {!isFunctionDisabled && expandedSection === "function" && (
                                <div className="filter-accordion-content px-4 pb-3 flex flex-col gap-1.5">
                                    {setSelectedFunction ? (
                                        // In-place radio filters
                                        <div className="flex flex-col gap-1.5 max-h-[200px] overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-[#c4a84f]/40 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-[#faf7f2]">
                                            <label className="flex cursor-pointer items-center gap-2.5 rounded-[2px] px-1 py-1.5 hover:bg-[#faf7f2] transition-colors">
                                                <input
                                                    ...
                                                    onChange={() => setSelectedFunction(null)}
                                                    className="accent-[#c4a84f] w-3.5 h-3.5 cursor-pointer"
                                                />
                                                <span
                                                    className={`font-['Cormorant_Garamond',_Georgia,_serif] text-[13px] ${selectedFunction === null
                                                        ? "font-semibold text-[#c4a84f]"
                                                        : "text-[#3d2b00]"
                                                        }`}
                                                >
                                            ...
                                            {functions.map((fn) => (
                                                <label
                                                    key={fn._id || fn.id || fn.slug}
                                                    className="flex cursor-pointer items-center gap-2.5 rounded-[2px] px-1 py-1.5 hover:bg-[#faf7f2] transition-colors"
                                                >
                                                    <input
                                                        type="radio"
                                                        name="filter-function"
                                                        checked={selectedFunction === fn.slug}
                                                        onChange={() => setSelectedFunction(fn.slug)}
                                                        className="accent-[#c4a84f] w-3.5 h-3.5 cursor-pointer"
                                                    />
                                                    <span
                                                        className={`font-['Cormorant_Garamond',_Georgia,_serif] text-[13px] ${selectedFunction === fn.slug
                                                            ? "font-semibold text-[#c4a84f]"
                                                            : "text-[#3d2b00]"
                                                            }`}
                                                    >
                                            ...
                                            {functions.length === 0 ? (
                                                ...
                                                functions.map((fn) => (
                                                    <Link
                                                        ...
                                                        onClick={() => setFilterOpen(false)}
                                                        className={`block rounded-[2px] px-2 py-1.5 font-['Cormorant_Garamond',_Georgia,_serif] text-[13px] no-underline transition-all hover:bg-[#faf7f2] hover:translate-x-0.5 ${fn.slug === currentFunctionSlug
                                                            ? "font-semibold text-[#c4a84f] bg-[#faf7f2]"
                                                            : "text-[#3d2b00]"
                                                            }`}
                                                    >
                                                        {fn.slug === currentFunctionSlug ? "✓ " : "○ "}
                                                        {fn.name}
                                                    </Link>
                                                ))
                                            )}
    ...
    return (
        <div className="mb-4 flex flex-wrap items-center gap-2">
            ...
            {selectedFunction && setSelectedFunction && (
                <span className="inline-flex items-center gap-1.5 rounded-full border border-[#c4a84f]/40 bg-[#c4a84f]/8 px-3 py-1 font-['Cormorant_Garamond',_Georgia,_serif] text-[12px] text-[#8b6914]">
                    {functions.find((f) => f.slug === selectedFunction)?.name || selectedFunction}
                    <button
